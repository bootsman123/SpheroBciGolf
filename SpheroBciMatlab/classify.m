function [] = classify(classifier, varargin)
% now apply this classifier to the new data
%
%  [] = classify(classifier, varargin)
%
% Options:
...
if ( isempty(trlen_samp) ) 
  if ( isfield(classifier,'windowFn') ) % est from size welch window function
    trlen_samp=size(classifier.windowFn,2); 
...
while( ~endTest )
  ...
  for si = 1:numel(start);    
    ...
    [f,fraw,p]=buffer_apply_ersp_clsfr(data.buf,classifier);
